...
{
	typename ImageAccessConcept::PixelType p;

	unsigned int w = img.Width();
	unsigned int h = img.Height();
	unsigned int s = img.Stride();

	const typename ImageAccessConcept::PixelType* ptr = img.RowPtr(0);
	p = img.Pixel(0, 0);
...
bool ComputeImageBBox(const ImageAccessType& img, IntType& left, IntType& top, IntType& right, IntType& bottom)
{
	...
	{
		const typename ImageAccessType::PixelType* pRow = img.RowPtr(y);
