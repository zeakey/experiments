...
parser.add_argument('--tmp', help='tmp folder', default="tmp/emd")
...
class WassersteinLoss(torch.autograd.Function):
    ...
    def forward(ctx, prediction, label, M, reg, numItermax=100, eps=1e-6):
        ...
        ctx.save_for_backward(u, torch.tensor([reg], dtype=M.dtype).cuda(),
                                 torch.tensor([bs], dtype=M.dtype).cuda())

        return loss_total.clone() / bs
    
    @staticmethod    
    def backward(ctx, grad_output):
        u, reg, bs = ctx.saved_tensors
        bs = bs.item()
        dim = u.size(1)
        ...
        return grad_input * (grad - shifting) * reg / bs, None, None, None, None, None
...
# loss function
wassloss = WassersteinLoss.apply
tmp1 = torch.arange(args.num_classes).repeat(args.num_classes, 1).float()
tmp2 = torch.arange(args.num_classes).unsqueeze(1).float()
gm = torch.abs(tmp1 - tmp2).cuda(device=args.gpu)
...
def train(train_loader, epoch):
    ...
    for i, (data, target) in enumerate(train_loader):
        ...
        output = model(data)
        loss = wassloss(output, target, gm, 10)
