...
def parse_args():
    ...
    parser.add_argument('--save-dir', type=str, default='tmp/imagenet',
...
def main():
    ...
    def train(ctx):
        if isinstance(ctx, mx.Context):
            ctx = [ctx]
        if opt.resume_params is '' and not opt.use_pretrained:
        ...
        for epoch in range(opt.resume_epoch, opt.num_epochs):
            ...
            err_top1_before, err_top5_before = test(ctx, val_data)
            mask.prune_param()
            ...
            tfboard_writer.add_scalar('val/epoch-acc5-before', 1-err_top5_before, epoch)
